...
import sys
sys.path.append('.')
from bop_toolkit_lib import pose_error, renderer, inout
from decimal import Decimal

detect_fail = {'ape': 15, 'can': 19, 'cat': 65, 'driller': 2, 'duck': 14, 'eggbox': 241, 'glue': 51, 'holepuncher': 1}
#detect_fail = {'ape': 0, 'can': 0, 'cat': 0, 'driller': 0, 'duck': 1, 'eggbox': 0, 'glue': 0, 'holepuncher': 0}

class Evaluation(object):
    def __init__(self, cfg, models_info, models, models_syms):
        self.models_info = models_info
        self.models = models
        self.models_syms = models_syms
        self.pose_est_all = {}
        self.pose_gt_all = {}
        self.depth_test_pth_all = {}
        self.num = {}
        self.numAll = 0.
    ...
    def evaluate_pose_add(self, output_dir):
        ...
        for i, cls_name in enumerate(self.classes):
            ...
            area = simps(count_correct['mean'][i] / float(count_all[i] + detect_fail[cls_name]), dx=dx) / 0.1
            acc_mean = area * 100
            sum_acc_mean[0] += acc_mean
            acc_002 = 100 * float(count_correct['0.02'][i]) / float(count_all[i] + detect_fail[cls_name])
            sum_acc_002[0] += acc_002
            acc_005 = 100 * float(count_correct['0.05'][i]) / float(count_all[i] + detect_fail[cls_name])
            sum_acc_005[0] += acc_005
            acc_010 = 100 * float(count_correct['0.10'][i]) / float(count_all[i] + detect_fail[cls_name])
            sum_acc_010[0] += acc_010

            fig = plt.figure()
            x_s = np.arange(0, 0.1, dx).astype(np.float32)
            y_s = count_correct['mean'][i] / float(count_all[i] + detect_fail[cls_name])
            ...
            logger.info('threshold=0.02, correct poses: {}, all poses: {}, accuracy: {:.2f}'.format(
                ...
                count_all[i] + detect_fail[cls_name],
                acc_002))
    ...
    def evaluate_pose_arp_2d(self, output_dir):
        ...
        dx = 0.1
        threshold_mean = np.tile(np.arange(0, 10.1, dx).astype(np.float32),
        ...
        for i, cls_name in enumerate(self.classes):
            ...
            logger.info("** {} **".format(cls_name))
            from scipy.integrate import simps
            area = simps(count_correct['mean'][i] / float(count_all[i] + detect_fail[cls_name]), dx=dx) / (10.0)
            acc_mean = area * 100
            sum_acc_mean[0] += acc_mean
            acc_02 = 100 * float(count_correct['2'][i]) / float(count_all[i] + detect_fail[cls_name])
            sum_acc_02[0] += acc_02
            acc_05 = 100 * float(count_correct['5'][i]) / float(count_all[i] + detect_fail[cls_name])
            sum_acc_05[0] += acc_05
            acc_10 = 100 * float(count_correct['10'][i]) / float(count_all[i] + detect_fail[cls_name])
            sum_acc_10[0] += acc_10
            acc_20 = 100 * float(count_correct['20'][i]) / float(count_all[i] + detect_fail[cls_name])
            sum_acc_20[0] += acc_20
            '''
            fig = plt.figure()
            x_s = np.arange(0, 50, dx).astype(np.float32)
            y_s = 100 * count_correct['mean'][i] / float(count_all[i] + detect_fail[cls_name])
            plot_data[cls_name].append((x_s, y_s))
            plt.plot(x_s, y_s, '-')
            plt.xlim(0, 50)
            plt.ylim(0, 100)
            ...
            plt.savefig(os.path.join(output_dir, 'arp_2d_widerange_{}.png'.format(cls_name)), dpi=fig.dpi)
            plt.close()
            '''
            fig = plt.figure()
            x_s = np.arange(0, 10.1, dx).astype(np.float32)
            y_s = 100 * count_correct['mean'][i] / float(count_all[i] + detect_fail[cls_name])
            plot_data[cls_name].append((x_s, y_s))
            plt.plot(x_s, y_s, '-')
            plt.xlim(0, 10)
            ...
            logger.info('threshold=2, correct poses: {}, all poses: {}, accuracy: {:.2f}'.format(
                count_correct['2'][i], count_all[i] + detect_fail[cls_name], acc_02))
            logger.info('threshold=5, correct poses: {}, all poses: {}, accuracy: {:.2f}'.format(
                count_correct['5'][i], count_all[i] + detect_fail[cls_name], acc_05))
            logger.info(
                'threshold=10, correct poses: {}, all poses: {}, accuracy: {:.2f}'.format(
                    count_correct['10'][i], count_all[i] + detect_fail[cls_name], acc_10))
            logger.info(
                'threshold=20, correct poses: {}, all poses: {}, accuracy: {:.2f}'.format(
                    count_correct['20'][i], count_all[i] + detect_fail[cls_name], acc_20))
            logger.info(" ")
    ...
    def evaluate_pose_vsd(self, output_dir):
        """
        Evaluate 6D pose by vsd Metric
        vsd(R_est, t_est, R_gt, t_gt, depth_test, K, delta, taus, normalized_by_diameter, diameter, renderer, obj_id, cost_type='step')
        """
        vsd_deltas_dic = {
            'lm': 15,
            'lmo': 15,
        }
        vsd_delta = vsd_deltas_dic[self.cfg.name]
        vsd_taus = np.linspace(0.05, 0.50, 10)
        vsd_taus = [Decimal(element).quantize(Decimal('0.00')) for element in vsd_taus]
        thetas = np.linspace(0.05, 0.50, 10)                                                                  # threshold
        thetas = [Decimal(element).quantize(Decimal('0.00')) for element in thetas]
        all_poses_est = self.pose_est_all
        all_poses_gt = self.pose_gt_all
        all_depth_test_pth = self.depth_test_pth_all
        models_info = self.models_info
        K = self.camera_matrix
        width = 640
        height = 480
        renderer_type = 'python'
        logger.info('\n* {} *\n {:^}\n* {} *'.format('-' * 100, 'Metric vsd', '-' * 100))
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        eval_method = 'vsd'
        num_classes = len(self.classes)
        count_all = np.zeros((num_classes), dtype=np.float32)
        count_correct = {k: np.zeros((num_classes), dtype=np.float32) for k in thetas}
        threshold = np.zeros((num_classes, len(thetas)),dtype=np.float32)
        for i in range(num_classes):
            for k in range(len(thetas)):
                threshold[i][k] = thetas[k]
        self.classes = sorted(self.classes)
        num_valid_class = len(self.classes)
        acc = {k: np.zeros((num_classes), dtype=np.float32) for k in thetas}
        sum_acc = {k: 0 for k in self.classes}
        mean_acc = {k: 0 for k in self.classes}

        for i, cls_name in enumerate(self.classes):
            curr_poses_gt = all_poses_gt[cls_name]
            curr_poses_est = all_poses_est[cls_name]
            curr_depth_test_pth = all_depth_test_pth[cls_name]
            diameter = models_info[ref.obj2idx(cls_name)]['diameter']
            normalized_by_diameter = True
            model_pth = os.path.join(ref.lmo_model_dir, cls_name, "{}.ply".format(cls_name))
            ren = renderer.create_renderer(width, height, renderer_type, mode="depth")
            ren.add_object(i, model_pth)
            num = len(curr_poses_gt)
            count_all[i] = num
            logger.info("** {} **".format(cls_name))
            for j in range(num):
                RT = curr_poses_est[j]  # est pose
                pose_gt = curr_poses_gt[j]  # gt pose
                depth_test_pth = curr_depth_test_pth[j]
                depth_test = inout.load_depth(depth_test_pth)
                normalized_by_diameter = True
                #error = add(RT[:3, :3], RT[:, 3], pose_gt[:3, :3], pose_gt[:, 3], models[cls_name])
                error_list = pose_error.vsd(RT[:3, :3], RT[:, 3].reshape(3, 1), pose_gt[:3, :3], pose_gt[:, 3].reshape(3, 1), depth_test, K, \
                                                   vsd_delta, vsd_taus, normalized_by_diameter, diameter, ren, i, cost_type='step')          # i = obj_id
                error = np.mean(error_list)
                for k, theta in enumerate(thetas):
                    if error < threshold[i][k]:
                        count_correct[theta][i] += 1

            for theta in thetas:
                acc[theta][i] = 100 * float(count_correct[theta][i]) / float(count_all[i] + detect_fail[cls_name])
                sum_acc[cls_name] += acc[theta][i]
                logger.info('threshold={}, correct poses: {}, all poses: {}, accuracy: {:.2f}'.format(
                    theta,
                    count_correct[theta][i],
                    count_all[i] + detect_fail[cls_name],
                    acc[theta][i]))
                logger.info(' ')

            mean_acc[cls_name] = sum_acc[cls_name] / len(thetas)
            logger.info('threshold={}, all poses: {}, accuracy: {:.2f}'.format(
                'mean',
                count_all[i] + detect_fail[cls_name],
                mean_acc[cls_name]))
            logger.info(' ')
    ...
    def evaluate_pose_mssd(self, output_dir):
        """
        Evaluate 6D pose by mssd Metric
        mssd(R_est, t_est, R_gt, t_gt, pts, syms)
        """
        models = self.models
        models_syms = self.models_syms
        thetas = np.linspace(0.05, 0.50, 10)                                                                  # threshold
        thetas = [Decimal(element).quantize(Decimal('0.00')) for element in thetas]
        all_poses_est = self.pose_est_all
        all_poses_gt = self.pose_gt_all
        ...
        for i, cls_name in enumerate(self.classes):
            ...
            for j in range(num):
                RT = curr_poses_est[j]  # est pose
                pose_gt = curr_poses_gt[j]  # gt pose
                error = pose_error.mssd(RT[:3, :3], RT[:, 3].reshape(3, 1), pose_gt[:3, :3], pose_gt[:, 3].reshape(3, 1), models[cls_name], models_syms[cls_name])
                for k, theta in enumerate(thetas):
                    if error < threshold[i][k] * diameter:
                        count_correct[theta][i] += 1
    ...
    def evaluate_pose_mspd(self, output_dir):
        """
        Evaluate 6D pose by mspd Metric
        mspd(R_est, t_est, R_gt, t_gt, K, pts, syms)
        """
        models = self.models
        models_syms = self.models_syms
        thetas = np.linspace(5, 50, 10)                                                                  # threshold
        all_poses_est = self.pose_est_all
        all_poses_gt = self.pose_gt_all
        K = self.camera_matrix
        width = 640
        ...
        for i, cls_name in enumerate(self.classes):
            ...
            for j in range(num):
                RT = curr_poses_est[j]  # est pose
                pose_gt = curr_poses_gt[j]  # gt pose
                error = pose_error.mspd(RT[:3, :3], RT[:, 3].reshape(3, 1), pose_gt[:3, :3], pose_gt[:, 3].reshape(3, 1), K, models[cls_name], models_syms[cls_name])
                for k, theta in enumerate(thetas):
                    if error < threshold[i][k] * r:
                        count_correct[theta][i] += 1
