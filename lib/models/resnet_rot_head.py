...
class RotHeadNet(nn.Module):
    def __init__(self, in_channels, num_layers=3, num_filters=256, kernel_size=3, output_kernel_size=1,
        ...
        for i in range(num_layers):
            _in_channels = in_channels if i == 0 else num_filters     # if i == 0: _in_channels = in_channels; else：_in_channels = num_filters
        ...
        if with_bias_end:
            self.features.append(
                nn.Conv2d(num_filters, output_dim, kernel_size=output_kernel_size, padding=pad, bias=True)) #actually,in model.py,you can see out_dim_real
        else:
            self.features.append(
