    ...
    for i, (obj, obj_id, inp, pose, c_box, s_box, box, trans_local) in enumerate(data_loader):
        ...
        for idx in range(len(col)):
            ...
            if 'trans' in cfg.pytorch.task.lower():
                ...
                T_vector_trans = np.asarray([pred_x, pred_y, pred_depth], dtype=np.double)
                print(T_vector_trans)
            ...
            if 'rot' in cfg.pytorch.task.lower():
                ...
                if cfg.test.pnp == 'iterPnP': # iterative PnP algorithm
                    ...
                    _, R_vector, T_vector, inliers = cv2.solvePnPRansacn(model_points, image_points,
                                            cfg.dataset.camera_matrix, dist_coeffs, T_vector_trans, iterationsCount = 200, reprojectionError=5.5, flags=cv2.SOLVEPNP_EPNP)
                else:
