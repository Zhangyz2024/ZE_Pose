...
from utils.eval_lmo import calc_all_errs, Evaluation
...
def test(epoch, cfg, data_loader, model, obj_vtx, obj_info, obj_syms, criterions):

    model.eval()
    Eval = Evaluation(cfg.dataset, obj_info, obj_vtx, obj_syms)
    if 'trans' in cfg.pytorch.task.lower():
        Eval_trans = Evaluation(cfg.dataset, obj_info, obj_vtx, obj_syms)
    '''
    ...
    for i, (obj, obj_id, inp, pose, c_box, s_box, box, trans_local, depth_test_pth) in enumerate(data_loader):
        ...
        if int(i) % cfg.test.disp_interval == 0:
            # input image
        ...
        if 'rot' in cfg.pytorch.task.lower():
            ...
            if cfg.train.split_num > 1:
                pred_conf_x1 = np.zeros(bs)
                pred_conf_y1 = np.zeros(bs)
                pred_conf_z1 = np.zeros(bs)
            if cfg.train.split_num > 2:
                pred_conf_x2 = np.zeros(bs)
                pred_conf_y2 = np.zeros(bs)
                pred_conf_z2 = np.zeros(bs)
            if cfg.train.split_num > 4:
                pred_conf_x3 = np.zeros(bs)
                pred_conf_y3 = np.zeros(bs)
                pred_conf_z3 = np.zeros(bs)
        ...
        if cfg.train.split_num == 1:
            ...
            col = list(zip(obj, obj_id.numpy(), pred_coor, pred_conf, pred_conf_x1, pred_conf_y1, pred_conf_z1, pred_trans, pose.numpy(), c_box.numpy(), s_box.numpy(), box.numpy(), depth_test_pth))
            ...
            col = list(zip(obj, obj_id.numpy(), pred_coor, pred_conf, pred_conf_x1, pred_conf_y1, pred_conf_z1, pred_conf_x2, pred_conf_y2, pred_conf_z2, \
                                                                                            pred_trans, pose.numpy(), c_box.numpy(), s_box.numpy(), box.numpy(), depth_test_pth))
        elif cfg.train.split_num == 8:
            col = list(zip(obj, obj_id.numpy(), pred_coor, pred_conf, pred_conf_x1, pred_conf_y1, pred_conf_z1, pred_conf_x2,pred_conf_y2, pred_conf_z2, \
                                                  pred_conf_x3, pred_conf_y3, pred_conf_z3, pred_trans, pose.numpy(), c_box.numpy(), s_box.numpy(), box.numpy(), depth_test_pth))

        for idx in range(len(col)):
            if cfg.train.split_num == 1:
                obj_, obj_id_, pred_coor_, pred_conf_, pred_trans_, pose_gt, c_box_, s_box_, box_, depth_test_pth_ = col[idx]
            elif cfg.train.split_num == 2:
                obj_, obj_id_, pred_coor_, pred_conf_, pred_conf_x1_, pred_conf_y1_, pred_conf_z1_, pred_trans_, pose_gt, c_box_, s_box_, box_, depth_test_pth_ = col[idx]
            elif cfg.train.split_num == 4:
                obj_, obj_id_, pred_coor_, pred_conf_, pred_conf_x1_, pred_conf_y1_, pred_conf_z1_, pred_conf_x2_, pred_conf_y2_, pred_conf_z2_, \
                                                                                                    pred_trans_, pose_gt, c_box_, s_box_, box_, depth_test_pth_ = col[idx]
            elif cfg.train.split_num == 8:
                obj_, obj_id_, pred_coor_, pred_conf_, pred_conf_x1_, pred_conf_y1_, pred_conf_z1_, pred_conf_x2_, pred_conf_y2_, pred_conf_z2_, \
                                                       pred_conf_x3_, pred_conf_y3_, pred_conf_z3_, pred_trans_, pose_gt, c_box_, s_box_, box_, depth_test_pth_ = col[idx]
            T_begin = time.time()
            if 'rot' in cfg.pytorch.task.lower():
            ...
            try:
                if 'rot' in cfg.pytorch.task.lower():
                    ...
                    Eval.depth_test_pth_all[obj_].append(depth_test_pth_)
                    Eval.num[obj_] += 1
                    Eval.numAll += 1
        ...
        Bar.suffix = 'test Epoch: [{0}][{1}/{2}]| Total: {total:} | ETA: {eta:} | Loss {loss.avg:.4f} | Loss_rot {loss_rot.avg:.4f}'.format(
            epoch, i, num_iters, total=bar.elapsed_td, eta=bar.eta_td, loss=Loss, loss_rot=Loss_rot)
        bar.next()
    ...
    if 'rot' in cfg.pytorch.task.lower():
        logger.info("{} Evaluate of Rotation Branch of Epoch {} {}".format('-'*20, epoch, '-'*20))
        ...
        if 'all' in cfg.test.test_mode.lower():
            ...
            Eval.evaluate_pose_vsd(epoch_save_path)
            Eval.evaluate_pose_mssd(epoch_save_path)
            Eval.evaluate_pose_mspd(epoch_save_path)
        else:
            ...
            if 'proj' in cfg.test.test_mode.lower():
                Eval.evaluate_pose_arp_2d(epoch_save_path)
            if 'vsd' in cfg.test.test_mode.lower():
                Eval.evaluate_pose_vsd(epoch_save_path)
            if 'mssd' in cfg.test.test_mode.lower():
                Eval.evaluate_pose_mssd(epoch_save_path)
            if 'mspd' in cfg.test.test_mode.lower():
                Eval.evaluate_pose_mspd(epoch_save_path)

    bar.finish()
    return {'Loss': Loss.avg, 'Loss_rot': Loss_rot.avg}, preds