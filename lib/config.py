...
def get_default_config_pytorch():
    ...
    config.gpu = 0
    config.threads_num = 6         # 'nThreads'
    config.debug = False
    config.demo = '../demo'         # demo  path
...
def get_default_dataset_config():
    ...
    config.pbr_num = 3000
    config.pbr_samp_type = 'uniform'  # 'uniform' | 'random'
    return config
...
def get_default_train_config():
    config = edict()
    config.split_num = 1
    config.mask_xyz_cof = 1
    config.adjusted_mask = False
...
def get_default_loss_config():
    ...
    config.trans_local_loss_type = 'L2'
    config.trans_prob_loss_type = 'BCE'
    config.trans_local_loss_weight = 1
    config.trans_prob_loss_weight = 1
    config.trans_all_loss_weight = 1
    return config
...
def get_default_test_config():
    ...
    config.transwap = False
    config.pnp = 'ransac'  # 'ransac' | 'ePnP' | 'iterpnp'
    config.mask_threshold = 0.5
    config.mask_low_th = 0.1
    config.mask_high_th =0.9
...
class config():
    ...
    def parse(self, cls_name):
        ...
        # save path
        if cls_name == 'lm':
            config.pytorch['save_path'] = os.path.join(ref.exp_dir, config.pytorch.exp_id, 'lm_train')
        else:
            config.pytorch['save_path'] = os.path.join(ref.exp_dir, config.pytorch.exp_id, 'N_train', cls_name)
        if not os.path.exists(config.pytorch.save_path):
            os.makedirs(config.pytorch.save_path, exist_ok=True)
        # debug path
        if config.pytorch.debug:
            config.pytorch.threads_num = 6
